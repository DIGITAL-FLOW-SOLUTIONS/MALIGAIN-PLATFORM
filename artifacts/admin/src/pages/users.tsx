...
function UserDetailModal({ userId, onClose, onDeleted }: { userId: number; onClose: () => void; onDeleted: () => void }) {
  ...
  if (isLoading) return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
      <div className="bg-card rounded-xl p-8"><div className="animate-spin rounded-full h-8 w-8 border-b border-border-2 border-primary mx-auto" /></div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div className="bg-card rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
        ...
        <div className="flex border-b border-border px-2 overflow-x-auto">
          {TABS.map(({ key, label }) => (
            <button key={key} onClick={() => setTab(key)} className={`px-4 py-3 text-sm font-medium border-b border-border-2 transition-colors whitespace-nowrap ${tab === key ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground/80"}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {tab === "info" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><span className="text-muted-foreground">ID</span><p className="font-medium mt-0.5">#{user.id}</p></div>
                <div><span className="text-muted-foreground">Status</span><div className="mt-0.5"><StatusBadge status={user.status} /></div></div>
                <div><span className="text-muted-foreground">Username</span><p className="font-medium mt-0.5">{user.username}</p></div>
                <div><span className="text-muted-foreground">Email</span><p className="font-medium mt-0.5">{user.email}</p></div>
                <div><span className="text-muted-foreground">Phone</span><p className="font-medium mt-0.5">{user.phone ?? "—"}</p></div>
                <div><span className="text-muted-foreground">Country</span><p className="font-medium mt-0.5">{user.country ?? "—"}</p></div>
                <div><span className="text-muted-foreground">Referral Code</span><p className="font-mono text-xs bg-muted px-2 py-1 rounded mt-0.5 inline-block">{user.referralCode}</p></div>
                <div><span className="text-muted-foreground">Joined</span><p className="font-medium mt-0.5">{new Date(user.createdAt).toLocaleDateString()}</p></div>
                <div><span className="text-muted-foreground">Total Referrals</span><p className="font-medium mt-0.5">{totalReferrals}</p></div>
                <div><span className="text-muted-foreground">Tasks Done</span><p className="font-medium mt-0.5">{taskCompletions}</p></div>
              </div>

              <div className="pt-2">
                <p className="text-sm font-medium text-foreground/80 mb-2">Change Status</p>
                <div className="flex gap-2 flex-wrap">
                  {["active", "inactive", "suspended"].map(s => (
                    <button key={s} onClick={() => statusMut.mutate({ status: s })} disabled={user.status === s || statusMut.isPending}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${user.status === s ? "bg-muted text-muted-foreground border-border cursor-default" : "border-input hover:bg-muted/30 text-foreground/80"}`}>
                      Set {s}
                    </button>
                  ))}
                </div>
          ...
          {showDeleteConfirm && (
            <div className="fixed inset-0 z-[60] bg-black/50 flex items-center justify-center p-4">
              <div className="bg-card rounded-xl w-full max-w-sm shadow-2xl p-6 space-y-4">
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Are you sure you want to permanently delete <span className="font-semibold text-foreground">{user.username}</span>? This will remove all their data including wallet, transactions, and task history. This action cannot be undone.
                  </p>
                </div>
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 py-2 border border-input rounded-lg text-sm font-medium text-foreground/80 hover:bg-muted/30 transition-colors disabled:opacity-50">
                    Cancel
                  </button>
                  <button
          ...
          {tab === "edit" && (
            <div className="space-y-4">
              <p className="text-xs text-muted-foreground mb-2">ID, Joined date, Tasks Done and Referral Code cannot be edited.</p>
              <div className="space-y-3">
                ...
                {/* Referred By (Upline) */}
                <div>
                  <label className="block text-xs font-medium text-muted-foreground mb-1">Referred By (Upline)</label>
                        ...
                        {uplineQuery.length >= 2 && showUplineDrop && (
                          <div className="absolute z-10 left-0 right-0 mt-1 bg-card border border-border rounded-lg shadow-lg overflow-hidden">
                            {(uplineResults?.users ?? []).filter(u => u.id !== userId).length === 0 ? (
                              <p className="px-3 py-2.5 text-xs text-muted-foreground">No users found</p>
                            ) : (
              ...
              <button onClick={() => updateMut.mutate()} disabled={updateMut.isPending}
                className="w-full py-2.5 bg-primary hover:bg-primary/90 disabled:opacity-50 text-white rounded-lg text-sm font-medium transition-colors mt-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div key={label} className="bg-muted/30 rounded-lg p-3">
                    ...
                    <p className="font-bold text-foreground mt-0.5">KES {val.toFixed(2)}</p>
                  </div>
                ))}
              </div>

              <div className="border-t pt-4">
                <p className="text-sm font-medium text-foreground/80 mb-3">Adjust Wallet</p>
                <div className="space-y-3">
          ...
          {tab === "referrals" && (
            <div className="space-y-4">
              {!referralData ? <div className="text-center py-8 text-muted-foreground text-sm">Loading…</div> : (
                <>
                  <div className="text-sm text-muted-foreground">Total: <span className="font-bold text-foreground">{referralData.totalCount}</span> referrals across 3 levels</div>
                  {([1, 2, 3] as const).map(lvl => {
                    const list = lvl === 1 ? referralData.level1 : lvl === 2 ? referralData.level2 : referralData.level3;
                    return (
                      <div key={lvl}>
                        <p className="text-xs font-semibold text-muted-foreground uppercase mb-2">Level {lvl} ({list.length})</p>
                        <div className="space-y-1.5">
                          {list.length === 0 ? <p className="text-xs text-muted-foreground">None</p> : list.map(u => (
                            <div key={u.id} className="flex items-center justify-between bg-muted/30 rounded-lg px-3 py-2 text-sm">
          ...
          {tab === "password" && (
            <div className="space-y-4 max-w-sm mx-auto">
              <p className="text-sm text-muted-foreground">Set a new password for <span className="font-semibold text-foreground">{user.username}</span>. Minimum 6 characters.</p>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full px-3 py-2 pr-10 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary ${confirmPassword && !passwordsMatch ? "border-red-400 bg-red-50" : "border-border"}`}
                  />
              ...
              <button
                ...
                className="w-full py-2.5 bg-primary hover:bg-primary/90 disabled:opacity-50 text-white rounded-lg text-sm font-semibold transition-colors mt-2">
                ...
                {(data?.users ?? []).map((u: UserItem) => (
                  <tr key={u.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => setSelectedUser(u.id)} className="text-primary hover:text-primary p-1.5 rounded hover:bg-primary/10 transition-colors">
